...
async function setupUser(ctx, jwtPayload) {
    ...
    for (const projectDirectory of await glob(`app-report/${ctx.state.user.db}/*`)) {
        const project = projectDirectory.replace(`app-report/${db}/`, '');
        const url = `http://${ctx.host.replace('admin', 'report')}/${db}/${project}`;
        // displayed project name is project with hyphens replaced by spaces, capitalised
        const projectName = project.replace(/-/g, ' ').replace(/(^| )(\w)/g, s => s.toUpperCase());
        const projectNameSexualAssaultPatch = projectName.replace('Sexual Assault', 'Rape is a Crime'); // TODO: KLUDGE ALERT!

        try { // internal (single-page) form?
            await fs.access(`${projectDirectory}/templates/pages/page+.html`);
            const menu = projectNameSexualAssaultPatch + ' Internal Form';
            ctx.state.user.projects[menu] = url+'/*';
        } catch (e) { /* nothing to add to menu */ }

        try { // public form?
            await fs.access(`${projectDirectory}/templates/pages/index.html`);
            const menu = projectNameSexualAssaultPatch + ' Public Form';
            ctx.state.user.projects[menu] = url;
        } catch (e) { /* nothing to add to menu */ }
    }
    ...
    if (!global.db[db]) {
        try {
            ...
            ctx.flash = { username: ctx.state.user.email, loginfailmsg: e.message };
            ctx.redirect('/login');
            return;
        }
    }
