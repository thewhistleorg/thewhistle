...
class FormGenerator {
    ...
    static async build(org, project) {
        ...
        forms[`${org}/${project}`] = { inputs: {}, steps: {}, defaults: {}, types: {} };
        ...
        debug('build ==============================================', `${org}/${project}`, Math.ceil(t2-t1)+'ms');
    }
    ...
    static transformInput(org, project, page, input, parent) {
        ...
        switch (input.type) {
            ...
            case 'textbox':
                ...
                if (input.type == 'textbox') {
                    ...
                    if (topLevel) html += '</li>\n';
                } else { // text, number, tel, email, hidden
                    if (input.label) html += `<label for="${fullName}">${input.label}</label>`;
        ...
        // record input type in order to be able to indicate types on the report details page
        if (topLevel) {
            // note: ignore solo 'skip' radio button which would mask other input of same name
            if (input.name && !(input.options && Object.keys(input.options).toString() == 'skip')) forms[`${org}/${project}`].types[input.name] = input.type;
        } else {
            if (parent.name) forms[`${org}/${project}`].types[`${parent.name}-${input.name}`] = input.type;
        }
