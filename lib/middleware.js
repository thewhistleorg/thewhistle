...
async function setupUser(ctx, jwtPayload) {
    ...
    try {
        const org = ctx.state.user.db.replace(/-test$/, ''); // for the 'test' version of an org, get the canonical spec
        const reports = JSON.parse(await fs.readFile(`public/spec/${org}/submit-list.json`, 'utf8'));
        for (const report in reports) {
            const url = `http://${ctx.request.host.replace('admin', 'report')}/${db}/${report}`;
