    ...
    static transformInput(org, project, page, input, parent) {
        ...
        switch (input.type) {
            ...
            case 'textbox': {
                ...
                if (input.attributes) {   // eg min, max, step, pattern etc
                ...
                if (input.type == 'textbox') {
                    ...
                    const attributes = [ ...Object.entries(attrs) ].map(([ attrKey, attrVal ]) => `${attrKey}="${attrVal}"`);
                    if (input.label) html += `<label for="${fullName}">${input.label}</label>`;
                    html += `<input ${attributes.join(' ')} value="{{${fullName}}}" placeholder="${input.placeholder||''}" ${input.required ? 'required' : ''} class="${input.class||'pure-input-1'}">\n`;
                }
                break; }
            case 'radio':
            case 'checkbox':
