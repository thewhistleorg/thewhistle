...
class FormGenerator {
     ...
     *
     * @param {string} location - Base URL form specification JSON or YAML is to be found.
     * @param {string} org - Organisation report is for.
     * @param {string} project - Project (aka campaign).
     */
    static async build(location, org, project) {
        debug('FormGenerator.build', location, org, project);
        const t1 = Date.now();
        ...
        // check form spec exists, either as json or as yaml (for the 'test' version of an org, get
        // the canonical spec)
        const jsonSpecUrl = `${location}/${org.replace(/-test$/, '')}/${project}.json`;
        const yamlSpecUrl = `${location}/${org.replace(/-test$/, '')}/${project}.yaml`;
        const jsonSpecHead = await fetch(jsonSpecUrl, { method: 'HEAD' });
        const yamlSpecHead = await fetch(yamlSpecUrl, { method: 'HEAD' });
        if (!jsonSpecHead.ok && !yamlSpecHead.ok) {
            const e = new Error(`Form spec ${location}/${org}/${project} not found`);
            e.status = 404;
            throw e;
        }
        const specUrl = jsonSpecHead.ok ? jsonSpecUrl : yamlSpecUrl;

        // initialise global.forms, where info about inputs & steps is held
        global.forms[`${org}/${project}`] = { inputs: {}, steps: {} };

        try {
            const spec = await $RefParser.dereference(specUrl);
            ...
            const err = new Error(`Form spec ${location}/${org}/${project} build failed: ${e.message}`);
            err.status = 410; // Gone
            throw err;
        }
    }
            ...
            switch (itemType) {
                case 'text':
                    if (typeof item[itemType] == 'object') {
                        // alternative texts depending on previously entered value
                        const wrap = item[itemType].wrap || 'p'; // enclosing element
                        delete item[itemType].wrap;
                        const cssClass = item[itemType].class ? item[itemType].class+' ' : '';
                        delete item[itemType].class;
                        const text = item[itemType].text;
                        delete item[itemType].text;
                        if (Object.keys(item[itemType]).length == 0) {
                            // regular 'text' object with 'text' attribute
                            html += `<${wrap} class="${cssClass}">${text}</${wrap}>\n`;
                        } else {
                            // parameterised option texts: assume any one 'text' will depend on only one variable!
                            const keys = Object.keys(item[itemType]).map(v => v.split(':')[0]);
                            const [ key ] = [ ...new Set(keys) ];
                            const vals = Object.keys(item[itemType]).map(v => v.split(':')[1]);
                            const texts = Object.values(item[itemType]);
                            for (let i=0; i<vals.length; i++) {
                                html += `<${wrap} class="${cssClass}{{show ${key} '${vals[i]}'}}">${texts[i]}</${wrap}>\n`;
                            }
                        }
                        break;
                    } else {
    ...
    static transformInput(org, project, page, input, parent) {
        ...
        // get the label of the parent element:
        // ... convert top level options nodeList to array
        const topLevelOptions = topLevel ? [] : [ ...Object.entries(parent.options) ];
        // ... filter top level options to just parent of this input
        const [ parentOptions ] = topLevelOptions.filter(([ , option ]) => option.subsidiary && option.subsidiary.filter(el => el.input && el.input.name == input.name).length>0);
        // ... parentOptions[0] is the input name, parentOptions[1] is an object containing the label and the subsidiary
        const parentOptionLabel = topLevel ? '' : parentOptions[1].label;

        switch (input.type) {
            case 'text':
            case 'hidden':
                if (topLevel) rptPage[input.name] = { label: input.label || input.placeholder || '' };
                if (!topLevel) {
                ...
                break;
            case 'textbox':
                if (topLevel) rptPage[input.name] = { label: input.label };
                if (!topLevel) {
                    if (!rptPage[parent.name].subsidiary) rptPage[parent.name].subsidiary = {};
                    rptPage[parent.name].subsidiary[parentOptionLabel] = fullName;
                }
            ...
            case 'checkbox':
                ...
                break;
            case 'select':
                if (topLevel) rptPage[input.name] = { label: input.label || parent.label };
                if (!topLevel) {
                    if (!rptPage[parent.name].subsidiary) rptPage[parent.name].subsidiary = {};
                    rptPage[parent.name].subsidiary[parentOptionLabel] = fullName;
                }
        ...
        debug(`input${topLevel?' ':'>'}`, fullName, `(${input.type})`, input.label||'–');
        return html;
    }
