...
class FormGenerator {
    ...
    static async getFormat(org, project, page) {
        try {
            const formatJson = await fs.readFile(`public/format/${org}/${project}-${page}.json`, 'utf8');
            const ret = JSON.parse(formatJson);
            return ret;
        } catch (e) {
            return null;
        }
    ...
    static async build(org, project) {
        ...
        forms[`${org}/${project}`] = { inputs: {}, inputList: {}, steps: {}, defaults: {}, publish: {}, partials: {}, format: {}, pageBranches: {} };
        ...
        forms[`${org}/${project}`].incidentPages = dotProp.get(spec, 'options.incidentPages');

        if (spec.options['page-branching']) {
            forms[`${org}/${project}`]['page-branching'] = true;
            forms[`${org}/${project}`]['initial-pages'] = spec.options['initial-pages'];
        } else {
            forms[`${org}/${project}`]['page-branching'] = false;
        }

        if (!forms[`${org}/${project}`].incidentPages) {
            forms[`${org}/${project}`].incidentPages = Number.MAX_SAFE_INTEGER;
        }

        forms[`${org}/${project}`].newIncidentPage = dotProp.get(spec, 'options.newIncidentPage');

        if (!forms[`${org}/${project}`].newIncidentPage) {
            forms[`${org}/${project}`].newIncidentPage = 1;
        }
        ...
        for (const p in spec.pages) {
            ...
            forms[`${org}/${project}`].inputList[page] = {};
            ...
            if (spec.publish) forms[`${org}/${project}`].publish = spec.publish;

            forms[`${org}/${project}`].partials[page] = await FormGenerator.getPartials(spec.pages[p]);
            forms[`${org}/${project}`].format[page] = await FormGenerator.getFormat(org, project, page);

            if (forms[`${org}/${project}`]['page-branching']) {
                for (let i = 0; i < spec.pages[p].length; i++) {
                    const input = spec.pages[p][i].input;
                    if (input) {
                        for (const option in input.options) {
                            const branchPages = input.options[option]['page-branch'];
                            if (branchPages) {
                                if (!forms[`${org}/${project}`].pageBranches[input.name]) {
                                    forms[`${org}/${project}`].pageBranches[input.name] = {};
                                }
                                forms[`${org}/${project}`].pageBranches[input.name][input.options[option].label] = branchPages;
                            }
                        }
                    }
                }
    ...
    static transformInput(org, project, page, input, parent) {
        ...
        forms[`${org}/${project}`].inputList[page][input.name] = input.label;
        let html = '';
        ...
        switch (input.type) {
            ...
            case 'textbox': {
                ...
                // output html
                const attrs = { // all inputs have name, id
                    name: fullName,
                    id:   fullName,
                };
                if (input.attributes) {   // add any specified attributes eg min, max, step, pattern etc
                    for (const attr of input.attributes.split(' ')) {
                        const [ v, k ] = attr.split('=');
                        attrs[v] = k || 'true';
                    }
                }
                if (input.placeholder) { // add placeholder into attributes
                    attrs.placeholder = input.placeholder;
                }
                if (input.autocomplete) { // add autocomplete as 'list' attribute referencing datalist
                    attrs.list = `${attrs.id}-datalist`;
                    if (!input.placeholder) attrs.placeholder = '...'; // indicate datalist is being populated
                }
                if (input.type == 'textbox') {
                    ...
                    html += `<textarea ${attributes.join(' ')} ${input.required ? 'required' : ''} class="pure-input-1">{{${fullName}}}</textarea>\n`;
                    if (topLevel) html += '</li>\n';
                } else { // text, number, tel, url, email, hidden
                    attrs.type = input.type;
                    const attributes = [ ...Object.entries(attrs) ].map(([ attrKey, attrVal ]) => `${attrKey}="${attrVal}"`);
                    if (input.label) html += `<label for="${fullName}">${input.label}</label>`;
                    html += `<input ${attributes.join(' ')} value="{{${fullName}}}" ${input.required ? 'required' : ''} class="${input.class||'pure-input-1'}">\n`;
                }
                if (input.autocomplete) { // add the datalist element (to be populated by ajax for better load times)
                    html += `<datalist id="${attrs.id}-datalist"></datalist>`;
                }
                break; }
            case 'radio':
            case 'checkbox':
