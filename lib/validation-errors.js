/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
/* Server-side form validation to match browser html5 form validation (c) Chris Veness 2016-2017  */
/*                                                                                   MIT Licence  */
/*                                                                                                */
...
function validationErrors(body, validationRules) {
    ...
    const reWeek     = /^[wW](([0-4][0-9])|(5[0123]))$/;
    const reDatetime = /^\d{4}-\d{2}-\d{2}T[0-2][0-3]:[0-5][0-9]$/; // plus Date.parse()
    const reColor    = /^#[0-9a-f]{6}$/;
    const reUrl      = /^[a-z]+:/i; // !! - (almost) as applied in browsers
    ...
    for (const input in validationRules) {
        ...
        for (const attrName in rules[input]) {
            ...
            switch (attrName) {
                ...
                case 'pattern':
                    if (textTypes.includes(rules[input].type)
                        && !input.match(new RegExp(`^${attrValue}$`))) error = `“${input}” must match the format requested`;
                    break;
            }
        }
        ...
        if (!empty(value)) {
            ...
            switch (type) {
                ...
                case 'range':
                    ...
                    break;
                case 'date':
                    if (!value.match(reDate) || !Date.parse(value)) error = `“${input}” must be a month`;
                    break;
                case 'datetime-local':
                    if (!value.match(reDatetime) || !Date.parse(value)) error = `“${input}” must be a date-time`;
                    break;
                case 'time':
                ...
                case 'week':
                    ...
                    break;
                case 'email':
                    if (!isEmail.validate(value)) error = `“${input}” must be an e-mail`;
                    break;
                case 'tel': // no validation required
                    break;
                case 'url':
                    if (!input.match(reUrl)) error = `“${input}” must be a url`;
                    break;
                case 'color':
