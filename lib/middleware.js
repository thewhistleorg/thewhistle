...
async function setupUser(ctx, jwtPayload) {
    ...
    // having an '-internal.html' template, 'public' reports by having an 'index.html' template
    ...
    for (const project of await glob(`app-report/${ctx.state.user.db}/*`)) {
        ...
        if (internal) {
            const menu = project.replace(`app-report/${ctx.state.user.db}/`, '') + ' (internal)';
            const url = `http://${ctx.host.replace('admin', 'report')}${project.replace('app-report', '')}/internal`;
            ctx.state.user.projects[menu] = url;
        }
        const external = await fs.pathExists(`${project}/templates/pages/index.html`);
        if (external) {
            const menu = project.replace(`app-report/${ctx.state.user.db}/`, '') + ' (public)';
