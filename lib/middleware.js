...
async function setupUser(ctx, jwtPayload) {
    ...
    for (const projectDirectory of await glob(`app-report/${ctx.state.user.db}/*`)) {
        ...
        const internal = await fs.pathExists(`${projectDirectory}/templates/pages/page+.html`);
        if (internal) {
            const menu = projectNameSexualAssaultPatch + ' Internal Form';
            ctx.state.user.projects[menu] = url+'/*';
        }

        const external = await fs.pathExists(`${projectDirectory}/templates/pages/index.html`);
        if (external) {
