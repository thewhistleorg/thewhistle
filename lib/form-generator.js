...
import { Renderer } from 'koa-handlebars';         // handlebars templating
...
class FormGenerator {
    ...
    static async spec(org, project) {
        ...
        try {
            ...
            const spec = await $RefParser.dereference(`${org}/${project}`, { resolve: resolver }); //This throws an error
            ...
            // console.log('spec validn', org, project, valid.errors.join('; ').replace('instance.', ''))
        ...
        for (const org of organisations) {
            ...
            for (const project of projects) {
                try {
                    ...
                    console.error(`ERR building form spec ${org}/${project}:`, e.message);
                }
            }
        }
    ...
    static async build(org, project) {
        ...
        const renderer = new Renderer(); // handlebars renderer
        ...
        for (const p in spec.pages) {
            ...
            await renderer.compileTemplate(`${process.cwd()}/.generated-reports/${org}/${project}-${page}.html`);
        ...
        await renderer.compileTemplate(`${process.cwd()}/.generated-reports/${org}/${project}-+.html`);
    ...
    static transformBranch(org, project, page, val, parent, attrs) {
        const inputVal = attrs.value.replace(new RegExp(' ', 'g'), '_');
        let classList = attrs.name + '-' + inputVal + '-branch';
        if (!(val.attributes && val.attributes.split(' ').contains('checked'))) {
            classList += ' hide';
        }
        return `</ul><ul class="${classList}">\n${FormGenerator.transformBody(org, project, page, val.branch, parent)}\n`;
    }
        ...
        switch (input.type) {
            ...
            case 'checkbox':
                ...
                for (const [ key, val ] of Object.entries(input.options)) {
                    ...
                    html += '</li>\n';
                    if (val.branch) {
                        html += FormGenerator.transformBranch(org, project, page, val, parent, attrs);
                    }

                }
                // }
                html += '{{/checked}}\n';
                if (input.type!='button' && input.type!='recaptcha-button' && topLevel) {
                    html += '</ul>\n';
                }
                break;
            case 'select':
