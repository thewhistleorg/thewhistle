...
class Handlers {
    ...
    static async getPage(ctx) {
        ...
        if (!global.forms[`${org}/${project}`].inputs[ctx.params.page] && ctx.params.page!='*') {
 ...
 *
 * The input labels are obtained from global.forms, as set up by ReportGenerator.build().
 *
 * Since radio buttons / checkboxes with no value selected don't appear in post data, the processing
 * is driven by the global.forms (which is generated by ReportGenerator.build on first request to
 * report), and an n-dash recorded against that input.
 *
...
function formatReport(org, project, page, body) {
    const inputs = global.forms[`${org}/${project}`].inputs;
