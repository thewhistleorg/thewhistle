 ...
 * @param {Object} err - for 'error', the Error object.
 */
async function log(ctx, type, t1, t2, err) {
    // don't log development environment (note: if testing of logging required, can check for referer=mocha)
    if (ctx.app.env == 'development') { if (err && ctx.status==500) console.error(err); return; }
    ...
    // e-mail notification
    try {
        if (ctx.state==500) await Mail.sendText('cdv23@cam.ac.uk', 'The Whistle 500 error', err.stack);
    } catch (e) {
        console.error('log', e);
    }
}
